...
const generateAdaptivePlan = (
  ...
  for (let i = 0; i <= daysInterval; i++) {
    ...
    // Check for existing data to protect
    ...
    const getProtectedBlock = (time: string, defaultData: any) => {
      ...
      // Protection Rule: If it's done, edited, or has links, DON'T touch it.
      if (existing && (existing.status === 'done' || existing.isManuallyEdited || existing.youtubeUrl || existing.customLinkUrl)) {
        return existing;
      }
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10">
          <div className="flex flex-col gap-6 flex-1 min-w-0">
            ...
            <div className="space-y-2">
               <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent text-primary font-black text-[9px] uppercase tracking-widest shadow-xl shadow-accent/20 italic border border-accent/20"><Calendar className="h-3 w-3" /> COMMAND CENTER v49.0</div>
                      ...
                      {day.blocks?.map((block: any) => {
                        ...
                        return (
                          <Card 
                            key={block.id} 
                            className={cn(
                              "aspect-square p-6 rounded-[3rem] border-none transition-all hover:scale-[1.03] shadow-[0_40px_80px_-20px_rgba(15,23,42,0.15)] group relative overflow-hidden bg-white h-full flex flex-col justify-between",
           ...
           {editingBlock && (
             <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                   ...
                   <div className="space-y-3">
                      <Label className="text-[10px] font-bold uppercase opacity-40 ml-4">DERS LİNKİ</Label>
